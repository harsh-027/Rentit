...
app.get('/api/dashboard', auth, async (req: AuthRequest, res) => { const now = new Date(); const month = Number(req.query.month || now.getMonth() + 1); const year = Number(req.query.year || now.getFullYear()); const property = await prisma.property.findFirst({ where: { userId: req.userId! } }); if (!property) return res.json({ property: null, summary: { expected: 0, collected: 0, pending: 0, occupiedRooms: 0, totalRooms: 0 }, rent: [] }); const tenants = await prisma.tenant.findMany({ where: { userId: req.userId!, propertyId: property.id, status: 'active' }, include: { room: true, rents: { where: { month, year }, include: { payments: true } } } }); const rooms = await prisma.room.findMany({ where: { propertyId: property.id, status: { not: 'archived' } } }); const rent = tenants.map(t => { const record = t.rents[0]; const expected = Number(record?.rentAmount ?? t.monthlyRent); const paid = record?.payments.reduce((sum, p) => sum + Number(p.amount), 0) ?? 0; return { tenant: t, rentRecord: record, expected, paid, due: Math.max(expected - paid, 0), status: paid >= expected ? 'Paid' : paid > 0 ? 'Partial' : 'Pending' } }); const expected = rent.reduce((s, r) => s + r.expected, 0), collected = rent.reduce((s, r) => s + r.paid, 0); res.json({ property, summary: { expected, collected, pending: Math.max(expected - collected, 0), occupiedRooms: rooms.filter(r => r.status === 'occupied').length, totalRooms: rooms.length }, rent }) })

app.get('/api/reports/monthly', auth, async (req: AuthRequest, res) => { const month = Number(req.query.month || new Date().getMonth() + 1); const year = Number(req.query.year || new Date().getFullYear()); if (!Number.isInteger(month) || month < 1 || month > 12 || !Number.isInteger(year)) return error(res, 422, 'Invalid reporting period'); const payments = await prisma.payment.findMany({ where: { property: { userId: req.userId! }, month, year }, include: { tenant: true, room: true }, orderBy: { paymentDate: 'desc' } }); const total = payments.reduce((sum, payment) => sum + Number(payment.amount), 0); res.json({ month, year, total, payments }) })
app.get('/api/reports/yearly', auth, async (req: AuthRequest, res) => { const year = Number(req.query.year || new Date().getFullYear()); if (!Number.isInteger(year) || year < 2000 || year > 2200) return error(res, 422, 'Invalid reporting year'); const payments = await prisma.payment.findMany({ where: { property: { userId: req.userId! }, year }, select: { month: true, amount: true } }); const monthly = Array.from({ length: 12 }, (_, index) => ({ month: index + 1, total: payments.filter(payment => payment.month === index + 1).reduce((sum, payment) => sum + Number(payment.amount), 0) })); res.json({ year, monthly, total: monthly.reduce((sum, item) => sum + item.total, 0) }) })
